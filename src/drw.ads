...
package Drw is
   ...
   procedure Resize (Drw_Ctx : Context_Access; Width, Height : Natural);
   ...
   --  fontset, also storing it as Drw_Ctx.Fonts (drw_fontset_create()).
   --  Returns null if none of the fonts could be loaded.
   function Fontset_Create
     (Drw_Ctx : Context_Access; Fonts : Dwm_Types.Command) return Font_Access;
   ...
   function Fontset_Get_Width (Drw_Ctx : Context_Access; Txt : String) return Natural;

   --  Like Fontset_Get_Width, but never returns more than Max_Width
   ...
   procedure Color_Create (Drw_Ctx : Context_Access; Dest : access Xft_Thin.XftColor; Color_Name : String);

   --  Releases a color allocated by Color_Create (drw_clr_free()).
   procedure Color_Free (Drw_Ctx : Context_Access; Color : access Xft_Thin.XftColor);
   ...
   procedure Scheme_Free (Drw_Ctx : Context_Access; Scheme : in out Dwm_Types.Color_Scheme_Access);

   --  Points Drw_Ctx at a different loaded fontset without recreating
   --  it (drw_setfontset()).
   procedure Set_Fontset (Drw_Ctx : Context_Access; Set : Font_Access);

   --  Selects the color scheme subsequent Rect/Text calls draw with
   --  (drw_setscheme()).
   procedure Set_Scheme (Drw_Ctx : Context_Access; Scheme : Dwm_Types.Color_Scheme_Access);
   ...
   --  returns the pixel position just past the drawn text (drw_text()).
   --  If Pos_X, Pos_Y, Width and Height are all zero, nothing is drawn
   --  and Invert instead acts as an optional width clamp -- this is
   --  how Fontset_Get_Width(_Clamp) measure text without rendering it.
   function Text
   ...
   function Cursor_Create (Drw_Ctx : Context_Access; Shape : Xlib_Thin.C_UInt) return Cursor_Access;

   --  Releases a cursor created by Cursor_Create and sets it to null
   --  (drw_cur_free()).
   procedure Cursor_Free (Drw_Ctx : Context_Access; Cursor : in out Cursor_Access);
