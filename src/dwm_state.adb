package body Dwm_State is
   ...
   function Get_Net_Atom (Kind : Net_Atom_Kind) return Xlib_Thin.Atom is (Net_Atom_Var (Kind));
   procedure Set_Net_Atom (Kind : Net_Atom_Kind; Value : Xlib_Thin.Atom) is
   begin
      Net_Atom_Var (Kind) := Value;
   end Set_Net_Atom;

   function Get_Num_Lock_Mask return Xlib_Thin.C_UInt is (Num_Lock_Mask_Var);
   procedure Set_Num_Lock_Mask (Value : Xlib_Thin.C_UInt) is
   begin
      Num_Lock_Mask_Var := Value;
   end Set_Num_Lock_Mask;

   function Get_Root return Xlib_Thin.Window is (Root_Var);
   procedure Set_Root (Value : Xlib_Thin.Window) is
   begin
      Root_Var := Value;
   end Set_Root;
   ...
   function Get_Scheme (Kind : Dwm_Types.Scheme_Kind) return Dwm_Types.Color_Scheme_Access is
     (Scheme_Var (Kind));
   procedure Set_Scheme (Kind : Dwm_Types.Scheme_Kind; Value : Dwm_Types.Color_Scheme_Access) is
   begin
      Scheme_Var (Kind) := Value;
   end Set_Scheme;

   function Get_Screen return Xlib_Thin.C_Int is (Screen_Var);
   procedure Set_Screen (Value : Xlib_Thin.C_Int) is
   begin
      Screen_Var := Value;
   end Set_Screen;

   function Get_Screen_Height return Integer is (Screen_Height_Var);
   procedure Set_Screen_Height (Value : Integer) is
   begin
      Screen_Height_Var := Value;
   end Set_Screen_Height;

   function Get_Screen_Width return Integer is (Screen_Width_Var);
   procedure Set_Screen_Width (Value : Integer) is
   begin
   ...
   procedure Set_Selected_Monitor (Value : Dwm_Types.Monitor_Access) is
   begin
   ...
   procedure Set_Stext (Value : Dwm_Types.Client_Name_Strings.Bounded_String) is
   begin
      Stext_Var := Value;
   end Set_Stext;

   function Get_Wm_Atom (Kind : Wm_Atom_Kind) return Xlib_Thin.Atom is (Wm_Atom_Var (Kind));
   procedure Set_Wm_Atom (Kind : Wm_Atom_Kind; Value : Xlib_Thin.Atom) is
   begin
      Wm_Atom_Var (Kind) := Value;
   end Set_Wm_Atom;

   function Get_Wm_Check_Window return Xlib_Thin.Window is (Wm_Check_Window_Var);
   procedure Set_Wm_Check_Window (Value : Xlib_Thin.Window) is
   begin
   ...
   procedure Set_X_Error_Xlib (Value : Xlib_Thin.XErrorHandler) is
   begin
