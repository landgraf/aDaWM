...
package body Dwm_Bar is
   ...
   procedure Draw_Bar (Monitor : Dwm_Types.Monitor_Access) is
      Box_Pad : constant Natural := Dwm_State.Drw_Ctx.Fonts.Height / 9;
      Box_Width : constant Natural := Dwm_State.Drw_Ctx.Fonts.Height / 6 + 2;
   ...
   begin
      ...
      if Monitor = Dwm_State.Selected_Monitor then
         Drw.Set_Scheme (Dwm_State.Drw_Ctx, Dwm_State.Scheme (Dwm_Types.Scheme_Norm));
         Status_Width :=
           Text_Width (Dwm_Types.Client_Name_Strings.To_String (Dwm_State.Stext)) - Dwm_State.Left_Right_Pad + 2;
         Ignore := Drw.Text
           (Dwm_State.Drw_Ctx, Monitor.Work_Width - Status_Width, 0, Status_Width, Dwm_State.Bar_Height, 0,
      ...
      for Idx in Config.Tags'Range loop
         ...
         begin
            ...
            Ignore := Drw.Text
              (Dwm_State.Drw_Ctx, Cur_X, 0, Label_Width, Dwm_State.Bar_Height, Dwm_State.Left_Right_Pad / 2,
               Config.Tags (Idx).all, (if (Urgent and Bit) /= 0 then 1 else 0));
            if (Occupied and Bit) /= 0 then
               Drw.Rect
                 (Dwm_State.Drw_Ctx, Cur_X + Box_Pad, Box_Pad, Box_Width, Box_Width,
                  (if Monitor = Dwm_State.Selected_Monitor
                     and then Dwm_State.Selected_Monitor.Selected_Client /= null
                     and then (Dwm_State.Selected_Monitor.Selected_Client.Tags and Bit) /= 0
                   then 1 else 0),
      ...
      if Remaining_Width > Dwm_State.Bar_Height then
         if Monitor.Selected_Client /= null then
            Drw.Set_Scheme
              (Dwm_State.Drw_Ctx,
               Dwm_State.Scheme
                 (if Monitor = Dwm_State.Selected_Monitor then Dwm_Types.Scheme_Sel else Dwm_Types.Scheme_Norm));
            Ignore := Drw.Text
              (Dwm_State.Drw_Ctx, Cur_X, 0, Remaining_Width, Dwm_State.Bar_Height, Dwm_State.Left_Right_Pad / 2,
            ...
            Drw.Set_Scheme (Dwm_State.Drw_Ctx, Dwm_State.Scheme (Dwm_Types.Scheme_Norm));
            Drw.Rect (Dwm_State.Drw_Ctx, Cur_X, 0, Remaining_Width, Dwm_State.Bar_Height, 1, 1);
         end if;
      end if;
      Drw.Map (Dwm_State.Drw_Ctx, Monitor.Bar_Window, 0, 0, Monitor.Work_Width, Dwm_State.Bar_Height);
   end Draw_Bar;
      ...
      while Monitor /= null loop
         if Monitor.Bar_Window = Xlib_Thin.None then
            Monitor.Bar_Window := Xlib_Thin.XCreateWindow
              (Dwm_State.Display, Dwm_State.Root, Xlib_Thin.C_Int (Monitor.Work_X), Xlib_Thin.C_Int (Monitor.Bar_Y),
               Xlib_Thin.C_UInt (Monitor.Work_Width), Xlib_Thin.C_UInt (Dwm_State.Bar_Height), 0,
               Xlib_Thin.XDefaultDepth (Dwm_State.Display, Dwm_State.Screen), Xlib_Thin.Copy_From_Parent,
               Xlib_Thin.XDefaultVisual (Dwm_State.Display, Dwm_State.Screen),
               Xlib_Thin.CWOverrideRedirect or Xlib_Thin.CWBackPixmap or Xlib_Thin.CWEventMask,
               Attrs'Access);
            Ignore := Xlib_Thin.XDefineCursor
              (Dwm_State.Display, Monitor.Bar_Window, Dwm_State.Cursors (Dwm_State.Cursor_Normal).X_Cursor);
            Ignore := Xlib_Thin.XMapRaised (Dwm_State.Display, Monitor.Bar_Window);
            Ignore := Xlib_Thin.XSetClassHint (Dwm_State.Display, Monitor.Bar_Window, Class_Hint'Access);
         end if;
         Monitor := Monitor.Next;
      end loop;
   end Update_Bars;
   ...
   begin
      ...
      Draw_Bar (Dwm_State.Selected_Monitor);
   end Update_Status;
