...
package Dwm_Clients is
   ...
   procedure Apply_Rules (Client : Dwm_Types.Client_Access);
   ...
   --  Returns True if the result differs from Client's current
   --  geometry, i.e. whether Resize should actually apply it.
   ...
   --  (arrange()).
   procedure Arrange (Monitor : Dwm_Types.Monitor_Access);  --  null means "all monitors"
   ...
   --  clients (arrangemon()).
   procedure Arrange_Mon (Monitor : Dwm_Types.Monitor_Access);

   --  Prepends Client to its monitor's client list (attach()).
   procedure Attach (Client : Dwm_Types.Client_Access);

   --  Prepends Client to its monitor's focus-history stack
   --  (attachstack()).
   procedure Attach_Stack (Client : Dwm_Types.Client_Access);

   --  Removes Client from its monitor's client list (detach()).
   procedure Detach (Client : Dwm_Types.Client_Access);
   ...
   procedure Detach_Stack (Client : Dwm_Types.Client_Access);
   ...
   --  (configure()).
   procedure Configure (Client : Dwm_Types.Client_Access);
   ...
   --  Set_Focus (focus()).
   procedure Focus (Client : Dwm_Types.Client_Access);
   ...
   --  (unfocus()).
   procedure Unfocus (Client : Dwm_Types.Client_Access; Clear_Focus : Boolean);

   --  Gives Client actual X input focus (unless it is never-focus) and
   --  updates _NET_ACTIVE_WINDOW and WM_TAKE_FOCUS (setfocus()).
   procedure Set_Focus (Client : Dwm_Types.Client_Access);
   ...
   procedure Grab_Buttons (Client : Dwm_Types.Client_Access; Focused : Boolean);
   ...
   procedure Manage (Window : Xlib_Thin.Window; Attrs : Xlib_Thin.XWindowAttributes);
   ...
   procedure Unmanage (Client : Dwm_Types.Client_Access; Destroyed : Boolean);
   ...
   function Next_Tiled (Client : Dwm_Types.Client_Access) return Dwm_Types.Client_Access;
   ...
   procedure Pop (Client : Dwm_Types.Client_Access);
   ...
   procedure Resize_Client (Client : Dwm_Types.Client_Access; Pos_X, Pos_Y, Width, Height : Integer);
   ...
   procedure Restack (Monitor : Dwm_Types.Monitor_Access);
   ...
   procedure Send_Mon (Client : Dwm_Types.Client_Access; Monitor : Dwm_Types.Monitor_Access);

   --  Sets Client's ICCCM WM_STATE property (e.g. NormalState,
   --  WithdrawnState) (setclientstate()).
   procedure Set_Client_State (Client : Dwm_Types.Client_Access; State : Long_Integer);
   ...
   procedure Set_Full_Screen (Client : Dwm_Types.Client_Access; Fullscreen : Boolean);

   --  Sets Client's urgency flag and mirrors it into the window's
   --  ICCCM WM_HINTS urgency bit (seturgent()).
   procedure Set_Urgent (Client : Dwm_Types.Client_Access; Urgent : Boolean);
   ...
   --  bottom-up (showhide()).
   procedure Show_Hide (Client : Dwm_Types.Client_Access);
   ...
   function Send_Event (Client : Dwm_Types.Client_Access; Proto : Xlib_Thin.Atom) return Boolean;
   ...
   --  (updatesizehints()).
   procedure Update_Size_Hints (Client : Dwm_Types.Client_Access);

   --  Re-reads Client's title from _NET_WM_NAME, falling back to
   --  WM_NAME, or "broken" if neither is set (updatetitle()).
   procedure Update_Title (Client : Dwm_Types.Client_Access);
   ...
   --  (updatewindowtype()).
   procedure Update_Window_Type (Client : Dwm_Types.Client_Access);
   ...
   procedure Update_Wm_Hints (Client : Dwm_Types.Client_Access);

   --  Finds the managed Client owning window Window, searching every
   --  monitor's client list, or null if Window isn't managed
   --  (wintoclient()).
   function Win_To_Client (Window : Xlib_Thin.Window) return Dwm_Types.Client_Access;
