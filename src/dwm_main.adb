...
package body Dwm_Main is
   ...
   begin
      Dwm_State.Set_X_Error_Xlib (Xlib_Thin.XSetErrorHandler (Dwm_Clients.X_Error_Start'Access));
      Ignore := Xlib_Thin.XSelectInput
        (Dwm_State.Get_Display, Xlib_Thin.XDefaultRootWindow (Dwm_State.Get_Display),
   ...
   begin
      ...
      Dwm_State.Get_Selected_Monitor.Layout (Dwm_State.Get_Selected_Monitor.Sel_Lt) := Cleanup_Foo'Access;
      ...
      while Dwm_State.Get_Monitors /= null loop
         Dwm_Monitors.Cleanup_Mon (Dwm_State.Get_Monitors);
      end loop;
      for Kind in Dwm_State.Cursor_Kind loop
         declare
            Cursor : Drw.Cursor_Access := Dwm_State.Get_Cursor (Kind);
         begin
            Drw.Cursor_Free (Dwm_State.Get_Drw_Ctx, Cursor);
            Dwm_State.Set_Cursor (Kind, Cursor);
         end;
      end loop;
      for Kind in Dwm_Types.Scheme_Kind loop
         declare
            Scheme : Dwm_Types.Color_Scheme_Access := Dwm_State.Get_Scheme (Kind);
         begin
            Drw.Scheme_Free (Dwm_State.Get_Drw_Ctx, Scheme);
            Dwm_State.Set_Scheme (Kind, Scheme);
         end;
      end loop;
      Ignore := Xlib_Thin.XDestroyWindow (Dwm_State.Get_Display, Dwm_State.Get_Wm_Check_Window);
      declare
         Drw_Ctx : Drw.Context_Access := Dwm_State.Get_Drw_Ctx;
      begin
         Drw.Free (Drw_Ctx);
         Dwm_State.Set_Drw_Ctx (Drw_Ctx);
      end;
      Ignore := Xlib_Thin.XSync (Dwm_State.Get_Display, 0);
      Ignore := Xlib_Thin.XSetInputFocus
        (Dwm_State.Get_Display, Xlib_Thin.Pointer_Root, Xlib_Thin.RevertToPointerRoot, Xlib_Thin.Current_Time);
   ...
   begin
      ...
      Dwm_State.Set_Display (Xlib_Thin.XOpenDisplay (Interfaces.C.Strings.Null_Ptr));
      ...
      Run;
      Cleanup;
      Ignore := Xlib_Thin.XCloseDisplay (Dwm_State.Get_Display);
   end Main;
   ...
   begin
      Ignore := Xlib_Thin.XSync (Dwm_State.Get_Display, 0);
      while Dwm_State.Get_Running loop
         Ignore := Xlib_Thin.XNextEvent (Dwm_State.Get_Display, Event'Access);
   ...
   begin
      Ok := Xlib_Thin.XQueryTree
        (Dwm_State.Get_Display, Dwm_State.Get_Root, Dummy1'Access, Dummy2'Access, Wins'Access, Count'Access);
      if Ok = 0 then
         return;
      end if;
      for Idx in 0 .. Integer (Count) - 1 loop
         ...
         begin
            Ok := Xlib_Thin.XGetWindowAttributes (Dwm_State.Get_Display, Window, Attrs'Access);
            if Ok /= 0 and then Attrs.Override_Redirect = 0
              and then Xlib_Thin.XGetTransientForHint (Dwm_State.Get_Display, Window, Dummy1'Access) = 0
            then
   ...
   procedure Setup is
      ...
      Net_Atoms : aliased Dwm_State.Net_Atom_Array;
      Ignore : Xlib_Thin.C_Int;
      Ignore_Bool : Boolean;
      ...
      begin
         Result := Xlib_Thin.XInternAtom (Dwm_State.Get_Display, Name_Ptr, 0);
   ...
   begin
      ...
      Dwm_State.Set_Screen (Xlib_Thin.XDefaultScreen (Dwm_State.Get_Display));
      Dwm_State.Set_Screen_Width
        (Integer (Xlib_Thin.XDisplayWidth (Dwm_State.Get_Display, Dwm_State.Get_Screen)));
      Dwm_State.Set_Screen_Height
        (Integer (Xlib_Thin.XDisplayHeight (Dwm_State.Get_Display, Dwm_State.Get_Screen)));
      Dwm_State.Set_Root (Xlib_Thin.XRootWindow (Dwm_State.Get_Display, Dwm_State.Get_Screen));
      Dwm_State.Set_Drw_Ctx
        (Drw.Create
      ...
      Dwm_State.Set_Bar_Height (Dwm_State.Get_Drw_Ctx.Fonts.Height + 2);
      ...
      Dwm_State.Set_Default_Layout ((Dwm_Bindings.Layouts (1)'Access, Dwm_Bindings.Layouts (2)'Access));
      ...
      Dwm_State.Set_Wm_Atom (Dwm_State.WM_Delete, Atom ("WM_DELETE_WINDOW"));
      Dwm_State.Set_Wm_Atom (Dwm_State.WM_State, Atom ("WM_STATE"));
      Dwm_State.Set_Wm_Atom (Dwm_State.WM_Take_Focus, Atom ("WM_TAKE_FOCUS"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_Active_Window, Atom ("_NET_ACTIVE_WINDOW"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_Supported, Atom ("_NET_SUPPORTED"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_Name, Atom ("_NET_WM_NAME"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_State, Atom ("_NET_WM_STATE"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_Check, Atom ("_NET_SUPPORTING_WM_CHECK"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_Fullscreen, Atom ("_NET_WM_STATE_FULLSCREEN"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_Window_Type, Atom ("_NET_WM_WINDOW_TYPE"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_WM_Window_Type_Dialog, Atom ("_NET_WM_WINDOW_TYPE_DIALOG"));
      Dwm_State.Set_Net_Atom (Dwm_State.Net_Client_List, Atom ("_NET_CLIENT_LIST"));

      Dwm_State.Set_Cursor
        (Dwm_State.Cursor_Normal, Drw.Cursor_Create (Dwm_State.Get_Drw_Ctx, Xlib_Thin.XC_left_ptr));
      Dwm_State.Set_Cursor
        (Dwm_State.Cursor_Resize, Drw.Cursor_Create (Dwm_State.Get_Drw_Ctx, Xlib_Thin.XC_sizing));
      Dwm_State.Set_Cursor
        (Dwm_State.Cursor_Move, Drw.Cursor_Create (Dwm_State.Get_Drw_Ctx, Xlib_Thin.XC_fleur));

      for Kind in Dwm_Types.Scheme_Kind loop
         Dwm_State.Set_Scheme (Kind, Drw.Scheme_Create (Dwm_State.Get_Drw_Ctx, Config.Colors (Kind)));
      end loop;

      Dwm_Bar.Update_Bars;
      Dwm_Bar.Update_Status;

      Dwm_State.Set_Wm_Check_Window
        (Xlib_Thin.XCreateSimpleWindow (Dwm_State.Get_Display, Dwm_State.Get_Root, 0, 0, 1, 1, 0, 0, 0));
      Check_Win_Buf := Dwm_State.Get_Wm_Check_Window;
      Ignore := Xlib_Thin.XChangeProperty
        (Dwm_State.Get_Display, Dwm_State.Get_Wm_Check_Window, Dwm_State.Get_Net_Atom (Dwm_State.Net_WM_Check),
         Xlib_Thin.XA_WINDOW, 32, Xlib_Thin.PropModeReplace, Check_Win_Buf'Address, 1);
      Ignore := Xlib_Thin.XChangeProperty
        (Dwm_State.Get_Display, Dwm_State.Get_Wm_Check_Window, Dwm_State.Get_Net_Atom (Dwm_State.Net_WM_Name),
         Utf8string, 8, Xlib_Thin.PropModeReplace, Dwm_Name'Address, 3);
      Ignore := Xlib_Thin.XChangeProperty
        (Dwm_State.Get_Display, Dwm_State.Get_Root, Dwm_State.Get_Net_Atom (Dwm_State.Net_WM_Check),
         Xlib_Thin.XA_WINDOW, 32, Xlib_Thin.PropModeReplace, Check_Win_Buf'Address, 1);
      Net_Atoms := Dwm_State.Get_All_Net_Atoms;
      Ignore := Xlib_Thin.XChangeProperty
        (Dwm_State.Get_Display, Dwm_State.Get_Root, Dwm_State.Get_Net_Atom (Dwm_State.Net_Supported),
         Xlib_Thin.XA_ATOM, 32, Xlib_Thin.PropModeReplace, Net_Atoms (Net_Atoms'First)'Address,
         Interfaces.C.int (Net_Atoms'Length));
      Ignore := Xlib_Thin.XDeleteProperty
        (Dwm_State.Get_Display, Dwm_State.Get_Root, Dwm_State.Get_Net_Atom (Dwm_State.Net_Client_List));

      Attrs.Cursor_Id := Dwm_State.Get_Cursor (Dwm_State.Cursor_Normal).X_Cursor;
      Attrs.Event_Mask :=
      ...
      Ignore := Xlib_Thin.XChangeWindowAttributes
        (Dwm_State.Get_Display, Dwm_State.Get_Root, Xlib_Thin.CWEventMask or Xlib_Thin.CWCursor, Attrs'Access);
      Ignore := Xlib_Thin.XSelectInput (Dwm_State.Get_Display, Dwm_State.Get_Root, Attrs.Event_Mask);
